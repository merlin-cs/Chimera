...
# def mimetic_mutation(smt_file):
#     """
#     Perform mimetic mutation on an SMT file.
#     """
#     # Parse the SMT file
#     script, globs = parse_file(smt_file)
#     # Get the terms from the script
#     terms = script.op_occs + script.free_var_occs
#     # Get the patterns from the rules
#     patterns = [rule['Target'] for rule in rules]
#     mimetic_pattern = random.choice(patterns)
#     orig_term = random.choice(terms)
...
#                 rand_subterm = random.choice(orig_term.subterms)
#             else:
#                 rand_subterm = orig_term
...
#         mimetic_term = convert(mimetic_term, orig_term_sort, mimetic_term_sort)
#     if mimetic_term != orig_term:
#         for assert_cmd in script.assert_cmd:
#             assert_cmd.term.substitute(orig_term, mimetic_term)
#     return script
...
def perform_mutation(orig_term, mimetic_pattern, script, globs):
    ...
    if "_Const" not in mimetic_pattern and ";" in mimetic_pattern:
        ...
        if mimetic_pattern.startswith(":"):
            ...
            pattern_parts = mimetic_pattern.split(":")
            if len(pattern_parts) >= 2:
                mimetic_term = Const(pattern_parts[0], type=pattern_parts[1].replace("_Const", ""))
            else:
                # Fallback for malformed patterns
                mimetic_term = Const(mimetic_pattern, type="String")

    mimetic_term_sort = judge_type(mimetic_term, globs)
    if mimetic_term_sort != "Unknown" and orig_term_sort != "Unknown" and mimetic_term_sort != orig_term_sort:
        converted_term = convert(mimetic_term, mimetic_term_sort, orig_term_sort)
        if converted_term:
            mimetic_term = converted_term
        # If conversion fails, continue with original mimetic_term
    
    if mimetic_term != orig_term:
        for assert_cmd in script.assert_cmd:
            if hasattr(assert_cmd, 'term') and assert_cmd.term:
...
def get_random_subterm(orig_term):
    """Get a random subterm from the given term."""
    if hasattr(orig_term, 'subterms') and orig_term.subterms:
...
if __name__ == '__main__':
    ...
    # mimetic_mutation(args.input, args.output)
    for i in range(1, 100):
        for mimetic_iter in range(MIMETIC_TIMES):
            temp_file = temp_dir + f"temp{random.randint(1, 1000)}.smt2"
            mutation_flag = mimetic_mutation(seed_file, temp_file)
            # mutation_flag = False
            if not mutation_flag:
                orig_file_path = seed_file
            else:
                orig_file_path = temp_file
                seed_file = orig_file_path
        # copy the temp file to the output file
        shutil.copyfile(orig_file_path, output_file)
        with open(output_file, "r") as f:
            file_str = f.read()
        if "Rewrite" in file_str:
            print(f"Mutation {i} completed, output saved to {output_file}")
            break

    # s, g = parse_file(str(current_parent_dir) + '/test/case2.smt2')
    # terms = s.op_occs
    # for idx, term in enumerate(terms):
    #     print(term)
    #     print(judge_type(term, g))

    # file = str(current_parent_dir) + '/test/case6.smt2'
    # with open(file, "r") as f:
    #     file_str = f.read()
    # # print("old file_str\n", file_str)
    # new = mimetic_mutation(file, str(current_parent_dir) + '/test/new.smt2')
    # print(new)

    # s, g = parse_file(str(current_parent_dir) + '/test/case1.smt2')
    # terms = s.op_occs + s.free_var_occs
    # # for term in terms:
    # #     if str(term) == "(= n 0)":
    # #         print(term)
    # #         src_type = judge_type(term, g)
    # #         print(src_type)
    # #         print(convert(term, src_type, "Real"))
    # test_term = random.choice(terms)
    # print(test_term)
    # src_type = judge_type(test_term, g)
    # # print(judge_type(test_term, g))
    # print(src_type)
    # print(type(convert(test_term, src_type, "Real")))

    # s, _ = parse_file(str(current_parent_dir) + '/test/case1.smt2')
    # print(s)
    # print(parse_csv('../res/RewriteRule.csv'))


    # TestObj = Mutate(str(current_parent_dir) + '/test/case12.smt2')
    # print(TestObj.orig_script[137])
    # print(TestObj._get_same_subterm(TestObj.orig_script[137].assert_cmd[0].term))
    # # for key, value in TestObj.candidate_rewrites.items():
    # #     print(key)
    # #     print(TestObj.candidate_rewrites[key][1])
    # # print(list(TestObj.candidate_rewrites.keys()))
    # # print(TestObj.candidate_rewrites.keys())
    # for term in TestObj.terms:
    #     if str(term) == """(or (not (>= (* c0 0) (str.indexof "a0" (str.substr "a" (* c0 0) c0)))) (not (<= (* c0 0) (str.indexof "a0" (str.substr "a" (* c0 0) c0)))))""":
    #         break
    
    # print(TestObj._compare_term(term, TestObj.orig_script[137].assert_cmd[0].term))
    ...
    # rewrite_idx = 0
    # # print(target)
    # # print(TestObj.candidate_rewrites["0"])
    # # TestObj.mutate_term(TestObj.candidate_rewrites['0'][1][0])
    # print(TestObj.candidate_rewrites["(abs (* I0_5 I0_5))"][0])
    # print(TestObj.candidate_rewrites["(abs (* I0_5 I0_5))"][1][0])
    # print(TestObj.mutate_term(TestObj.candidate_rewrites[target][0], TestObj.candidate_rewrites[target][1][rewrite_idx]))
